  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        bat(script: '"H:\\Install\\nuget.exe" restore "%JENKINS_HOME%\\workspace\\%JOB_NAME:/=_%\\DevopsTest.sln"', label: 'Nuget Restore')
        bat(script: '"H:\\Programs\\VisualStudio\\2017\\MSBuild\\15.0\\Bin\\MSBuild.exe" "%JENKINS_HOME%\\workspace\\%JOB_NAME:/=_%\\DevopsTest.sln" /t:Clean;Build /p:Configuration=Release /p:DeployOnBuild=true /p:PublishProfile=FolderProfile', label: 'MsBuild')
      }
    }
    stage('Unit Test') {
      steps {
    ...
    stage('Deploy') {
      steps {
        ...
        bat(script: 'xcopy /S /Y "%JENKINS_HOME%\\workspace\\MyTestProjects_master\\DevopsTest\\bin\\Release\\Publish" "H:\\CodeWorkspace\\PrinceWorkSpace\\Deploy\\DevopsTest\\Test"', label: 'Deploy Files')
      }
    }
  }
}