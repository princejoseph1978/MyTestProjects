  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        bat(script: '"H:\\Programs\\VisualStudio\\2017\\MSBuild\\15.0\\Bin\\MSBuild.exe" "%JENKINS_HOME%\\workspace\\MyTestProjects_master\\DevopsTest.sln" /t:Clean;Build /p:Configuration=Release /p:DeployOnBuild=true /p:publishUrl="%JENKINS_HOME%\\workspace\\MyTestProjects_master\\Publish" ', label: 'MsBuild')
      }
    }
    stage('Unit Test') {
      steps {
